...
Expose a focused Model Context Protocol tool surface for ASA server operators. The MCP client talks to this server over stdio, and this server opens short-lived RCON connections to the configured ASA host when a tool is invoked.
...
4. RCON connection execution in `src/rcon.ts`.

Configuration is loaded once at startup from `src/config.ts`.
